Input =
  ...
  doMove: (direction, keyDown) ->
    ...
    if keyDown
      switch direction
        when "n" then c.setVelocity(new gf.Vector(0, -c.movespeed))
        when "s" then c.setVelocity(new gf.Vector(0, c.movespeed))
        when "e" then c.setVelocity(new gf.Vector(c.movespeed,0))
        when "w" then c.setVelocity(new gf.Vector(-c.movespeed,0))
